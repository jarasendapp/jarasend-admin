...
const SECURITY_ACTIONS = ['login', 'logout', 'passwordChanged', 'pinChanged', 'biometricEnabled', 'biometricLogin', 'sessionLocked']
const ACTION_LABELS = {
  login: 'Login', logout: 'Logout', passwordChanged: 'Password changed', pinChanged: 'PIN changed',
  biometricEnabled: 'Biometric enabled', biometricLogin: 'Biometric login', sessionLocked: 'Session locked',
}
...
export default function SecurityCenter() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled) setError(err.message || 'Could not load security events.')
      } finally {
  ...
  const filtered = useMemo(() => {
    ...
    const base = rows.filter((r) => {
      ...
      const matchesSearch = !q || r.profile?.full_name?.toLowerCase().includes(q) || r.profile?.phone?.toLowerCase().includes(q)
  ...
  return (
    <div style={{ padding: 28 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          <h1 style={{ fontSize: 22, marginBottom: 4 }}>Security center</h1>
          <p style={{ color: 'var(--slate)', fontSize: 13, marginTop: 0 }}>
            {loading ? 'Loading…' : `${rows.length} account-security events`}
          </p>
        </div>
        <input
          type="text"
          ...
          style={{ minWidth: 240, padding: '8px 14px', borderRadius: 20, border: '1px solid var(--divider)', fontSize: 12.5, fontFamily: 'inherit' }}
        />
      </div>
      ...
      <div className="no-print" style={{ margin: '16px 0', display: 'flex', gap: 8, alignItems: 'center' }}>
        ...
        <TableToolbar
          ...
          columns={[
            ...
            { label: 'Event', value: (r) => ACTION_LABELS[r.action] ?? r.action },
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12.5 }}>
          <thead>
            <tr style={{ background: '#FAFBFC', borderBottom: '1px solid var(--divider)' }}>
              {['Account', 'Phone', 'Event', 'Detail', 'Device', 'When'].map((h) => (
